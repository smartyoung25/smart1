...
const summary = [
  ...
  P('여섯째, 출하 타이밍 최적화를 구현하였다. 2018~2025년 실측 거래의 월별 단가 집계를 근거로 최적 출하월을 제시하며, 표본이 부족한 월은 추천에서 제외한다. 검증 과정에서 매출 오차의 주된 원인이 수확량이 아니라 단가 변동임을 확인하였다 — 수확량을 완전히 안다고 가정해도 매출은 32~56% 오차가 남는다(농가·시점별 단가 변동계수 34~51%).', { after: 170 }),
  ...
  P('중요한 확인 사항으로, 데이터 기반 환경 최적화의 전제인 ∂수확량/∂환경 관계는 현재 데이터로 추정되지 않는다(교차검증 R² −0.4~−1.0). 원인은 분석 기법이 아니라 데이터 구조로, 동일 농가가 환경 설정을 크게 변화시킨 이력이 없어 효과를 식별할 변동이 존재하지 않는다. 따라서 계획서 과업②(다)의 현장실증 설계가 선행되어야 한다.', { after: 170 }),
  P('단 이는 온도·습도·CO₂·일사를 월별로 집계하여 수확량과 대조한 한정된 범위의 결과이며, "환경 관리가 무의미하다"는 뜻이 아니다. 양액 관리(급액 EC·pH·배액률)는 데이터가 수집되지 않아 검증 대상에 포함되지 않았고, 시간 단위 조치(VPD 밴드 관리·관수 P1~P6)와 품질·병해·자원 절감도 측정하지 않았다. 관측 데이터에서 효과가 식별되지 않는 것과 효과가 없는 것은 다르다. 그 전까지 환경 처방은 통제 실험으로 확립된 재배 기준(전략표) 대비 편차 방식을 유지하며, 검증되지 않은 관계를 최적화 근거로 제시하지 않는다(제3장 제2절·제6장 제2절).', { after: 170 }),
...
const ch3 = [
  ...
  B('③ 지표를 높이는 변경이 예측력 향상을 뜻하는지 추가 검증하였다. 모델 선택 기준을 오차율에서 설명력(CV R²)으로 바꾸면 전 작목이 월별 모드로 이동하며 설명력이 급등하였으나(참외 −0.122→0.506, 완숙토마토 0.099→0.453, 방울토마토 −0.257→0.195), 기여 변수를 분해한 결과 상위 변수가 전월 수확량·수확량 이동평균·계절 주기·농가 과거평균뿐이고 환경·경영 변수는 포함되지 않았다. 이는 환경 관리의 효과가 아니라 수확량의 자기상관(전월 실적이 당월 실적을 설명하는 성질)을 학습한 것이며, 예측·계획 시점에는 전월 실측이 존재하지 않아 의사결정에 사용할 수 없다. 따라서 해당 변경을 채택하지 않고 종전 기준을 유지하였다.'),
  B('이 검증으로 당해연도 판정(검증 적용 2 · 조건부 2 · 폴백 2)이 재확인되었다. 아울러 동일한 착시가 재발하지 않도록 판정 근거와 재현 절차를 코드에 포함시켰다.'),
  ...
  TBL([3400, 1500, 4738], [
    ...
    ['출하 타이밍 최적화', '이행', '2018~2025 실측 거래의 월별 단가 집계로 최적 출하월 제시(G7 연동).\n표본 부족 월(n<1,000)은 추천에서 제외 — 딸기 10월 단가는 거래 24건이라 배제'],
    ['환경제어 최적화', '부분', '전략표 목표 대비 편차 처방·관수 P1~P6·에너지 절감은 구현.\n데이터 기반 최적화(∂수확량/∂환경 추정)는 미성립 — 온도·습도·CO₂·일사의 월별 집계 대비 수확량 범위에 한함.\n양액은 데이터 미수집으로 검증 대상 제외(하단 마 참조)'],
  ], [AlignmentType.LEFT, AlignmentType.CENTER, AlignmentType.LEFT]),
  T('과업②(다) 현장실증·최적화 이행 현황'),

  P('마. 데이터 기반 환경 최적화의 성립 요건 검증 — 범위와 결과', { bold: true, after: 80 }),
  P('환경제어 "최적화"는 환경 설정을 바꿨을 때 결과가 얼마나 변하는지(∂결과/∂환경)를 수치로 알아야 성립한다. 이를 당해연도에 검증하였다. 결론부터 적으면, 아래 한정된 범위에서 이 관계를 추정할 수 없음을 확인하였다.', { after: 110 }),
  P('※ 본 검증의 범위 — 아래 결과를 확대 해석하지 않도록 먼저 밝힌다', { bold: true, after: 80 }),
  TBL([2400, 3600, 3638], [
    ['구분', '검증한 것', '검증하지 않은 것'],
    ['투입 변수', '온도·습도·CO₂·일사(내외부)', '양액(급액 EC·pH·배액률·함수율) — 수집 항목에 없어 검증 자체가 불가'],
    ['집계 단위', '월별', '시간·일 단위 조치(VPD 밴드 관리, 관수 P1~P6 등)'],
    ['결과 변수', '수확량', '품질(당도·상품률)·병해 억제·자원(에너지·양액) 절감'],
    ['대상 작목', '시설 6작목', '노지 7작목'],
  ], [AlignmentType.LEFT, AlignmentType.LEFT, AlignmentType.LEFT]),
  T('환경 최적화 검증의 범위'),
  B('따라서 본 결과는 "환경 관리가 무의미하다"는 뜻이 아니다. 관측 데이터에서 효과가 식별되지 않는 것과 효과가 없는 것은 다르다. 특히 양액 관리는 관련 데이터가 수집되지 않아 검증 대상에조차 포함되지 않았다.'),
  B('실제로 보조 데이터(농진청 딸기 자료)에서는 환경 지표가 품질과 더 강한 관계를 보인다 — 누적 일사량과 당도의 상관이 +0.627로 수확량(+0.390)보다 높고, 관수량도 당도와 +0.301의 관계를 보인다. 즉 환경·관수 관리의 효과는 수확량보다 품질에서 먼저 나타날 가능성이 있으나, 본 검증의 결과 변수는 수확량뿐이었다.'),
  B('다만 위 상관에도 계절 교란이 섞여 있다. 적산온도–수확량 상관 +0.641은 같은 달 안에서 비교하면 +0.485로 줄어드는데, 겨울 촉성 재배에서 "봄이 오면 수확이 는다"는 계절 효과가 온도 효과로 오인되기 때문이다. 해당 자료에는 농가·연도 식별자가 없어 교란을 완전히 제거할 수 없으므로 참고 수준으로만 제시한다.'),
  B('본 과제의 환경 처방은 데이터 기반 최적화가 아니라 Priva·Grodan·Plant Empowerment 등 통제 실험과 현장 축적으로 확립된 재배 기준에 근거한다(제2절 환경관리 전략표). 본 검증은 그 기준의 타당성을 평가한 것이 아니며, 평가할 수 있는 설계도 아니다.'),
  B('기존 수확량 모델(M2)은 예측 정확도가 목적이어서 과거 수확량 변수(직전 수확량·농가 이력)가 설명력을 지배한다. 그 결과 환경 변수의 기여가 사실상 없다 — 내부온도 20℃와 28℃의 예측값이 동일하며, CO₂는 변수에 포함되지 않았다. 즉 "지난해 성과가 좋았던 농가가 올해도 좋다"를 학습한 모델로, 예측에는 유효하나 환경 의사결정에는 사용할 수 없다.'),
  B('이에 과거 수확량·생육 변수를 제외하고 환경 변수만으로 수확량을 설명하는 별도 모델을 구성하였다(생육은 환경의 매개변수이므로 함께 제외 — 포함 시 환경의 총효과가 차단된다).'),
  B('검증 과정에서 원천 환경 데이터가 1분 단위임을 확인하였다(2021년 딸기 기준 213,188건·33농가, 내부온도 99.1%·CO₂ 76.5% 수집). 기존 파이프라인은 이를 월평균으로 집계하고 있었다. 제품이 활용하는 지표(일사 적산·DLI·VPD 밴드·주야 온도차)는 분 단위 원천에서만 산출 가능하므로, 생리학적 월별 지표를 새로 구성하였다.'),
  TBL([3800, 2000, 3838], [
    ['검증 조건', '교차검증 R²', '해석'],
    ['월평균 환경 지표', '−0.493', '평균값 예측보다 나쁨'],
    ['1분 원천 → 생리학적 지표', '−0.403', '개선 미미'],
    ['+ 농가 고정효과 제거(within-farm)', '−0.986', '교란 제거 시 오히려 악화'],
  ], [AlignmentType.LEFT, AlignmentType.CENTER, AlignmentType.LEFT]),
  T('환경→수확량 관계 검증 결과 (딸기, 연도 단위 교차검증)'),
  B('다만 지표의 방향성은 정상화되었다. 월평균 지표에서는 내부온도와 수확량의 상관이 −0.274로 농학적 통념과 배치되었으나(딸기 겨울 촉성 재배에서 고온기 수확 감소가 온도 효과로 오인됨), 생리학적 지표에서는 주야 온도차 +0.185·누적 일사 +0.172로 정합한 방향을 보였다. 농가 고정효과 제거 후에도 방향은 유지되나(+0.12~0.13) 크기가 작다.'),
  B('원인은 분석 기법이 아니라 데이터 구조에 있다. 농가 고정효과를 제거하면 성능이 더 나빠지는데, 이는 동일 농가가 환경 설정을 크게 변화시킨 이력이 없기 때문이다(유효 397건·31농가·4개년). 관측 데이터에서는 농가가 환경을 일정하게 유지하므로 환경 효과를 식별할 변동 자체가 존재하지 않는다.'),
  B('따라서 검증한 범위(온도·습도·CO₂·일사 → 수확량)에서 데이터 기반 최적화는 계획서 과업②(다)의 현장실증으로만 가능하다 — 동일 농가에서 환경 조건을 의도적으로 달리하는 설계가 필요하다. 검증되지 않은 관계를 근거로 환경 처방을 제시하면 농가를 잘못된 조치로 이끌 수 있으므로, 해당 모델은 서비스에 반영하지 않았다.'),
  B('당해연도 환경 처방은 데이터 기반 최적화가 아니라 Priva·Plant Empowerment 등 확립된 재배 기준(환경관리 전략표)의 목표값 대비 실측 편차에 근거한다 — 제2절 참조.'),

  P('※ 환경·양액·생육 최적화의 정량 검증 종합', { bold: true, after: 80 }),
  P('앞의 수확량 검증(제2절)과 환경 검증(마)에서 산출한 계량 결과를 최적화 영역별로 종합하면 다음과 같다. 계량이 성립한 영역과 데이터 부재로 성립하지 못한 영역을 수치와 함께 구분한다.', { after: 110 }),
  TBL([2200, 2600, 3200, 1638], [
    ['최적화 영역', '계량 지표', '측정 결과', '검증 상태'],
    ['수확량(M2)', '작목별 교차검증 R²', '6작목 CV R² −0.257 ~ 0.386 (적용 2·조건부 2·폴백 2)', '계량 완료 (제2절 판정)'],
    ['환경→수확량', '환경변수 단독 CV R²', '월평균 −0.493 / 생리학적 −0.403 / 농가 고정효과 제거 −0.986', '계량 완료·관계 미식별 (마)'],
    ['환경→품질(보조자료)', '상관계수', '누적일사–당도 +0.627(수확량 +0.390), 관수량–당도 +0.301', '참고 (계절 교란 미통제)'],
    ['양액→결과', '급액 EC·pH·배액률·함수율', '수집 0건', '계량 불가 — 데이터 미수집'],
    ['생육(M1)·병해(M5)', '교차검증 R²', '미산출 (3계층 구현·CV 미실시)', '계량 대기 (제6장)'],
  ], [AlignmentType.LEFT, AlignmentType.LEFT, AlignmentType.LEFT, AlignmentType.LEFT]),
  T('환경·양액·생육 최적화의 정량 검증 종합'),
  B('계량 결과가 산출된 영역은 수확량(M2)과 환경→수확량뿐이며, 후자는 CV R²가 모두 0 이하(월평균 −0.493·생리학적 −0.403·농가 고정효과 제거 −0.986)로 관측 데이터에서 환경→수확량 인과가 식별되지 않았다. 유효 관측 397건·31농가·4개년에서 농가가 환경을 일정하게 유지해 식별할 변동이 없기 때문이다(마).'),
  B('양액(급액 EC·pH·배액률·함수율)은 수집 항목에 없어(수집 0건) 계량 검증이 원천 불가하다. 환경 변수는 1분 단위 원천이 존재하나(2021년 딸기 213,188건·33농가, 내부온도 99.1%·CO₂ 76.5%) 수확량과 연결할 관측 변동이 부족하다.'),
  B('생육(M1)·병해(M5)는 3계층으로 구현하였으나 당해연도 정량 검증(교차검증 지표 산출)은 M2에 한정하였고, M1 토마토 계열 과적합 개선은 잔여 과제다(제6장).'),
  B('따라서 환경·양액·생육 최적화의 계량화는 동일 농가에서 투입을 의도적으로 달리하는 현장실증(제6장 제2절)으로 관측 변동을 확보한 뒤 성립한다. 검증되지 않은 계수로 처방하면 농가를 잘못된 조치로 이끌 수 있어 서비스에 반영하지 않았다.'),

  P('바. 소득·비용 최적화 검증 현황', { bold: true, after: 80 }),
  P('최적화는 "무엇을 바꾸면 성과가 얼마 변하는가"를 수치로 알아야 성립한다. 수확량(다)은 예측모델로 검증하였고 환경→수확량 관계(마)는 성립 요건을 검증하였다. 이에 대응하여 소득·비용 최적화의 검증 성립 여부를 아래와 같이 정리한다.', { after: 110 }),
  TBL([1800, 2800, 1300, 3738], [
    ['최적화 영역', '구현 형태', '검증 성립', '사유·근거'],
    ['수확량(M2)', '3계층 예측모델', '성립', 'CV R² 게이트 판정 — 검증적용 2·조건부 2·폴백 2 (제2절)'],
    ['환경', '목표값 대비 편차 처방', '미성립', '∂수확량/∂환경 데이터 식별 불가 — 관측상 환경 변동 부재 (마)'],
    ['소득', 'ERP 소득구조·BEP 집계', '미성립', '상류 전제(환경→수확량 인과) 미검증, 회계 집계이지 최적화 모델 아님'],
    ['비용', 'ERP 경영비 13항목·ROI', '부분', '경영비 이상치 탐지 미착수, CAPEX/OPEX 다년 ROI 미반영'],
  ], [AlignmentType.LEFT, AlignmentType.LEFT, AlignmentType.CENTER, AlignmentType.LEFT]),
  T('최적화 영역별 검증 성립 현황'),
  B('소득·비용 최적화가 독립 검증 모델로 성립하지 않는 근본 원인은 최적화의 상류 고리인 "환경→수확량" 인과가 관측 데이터에서 식별되지 않기 때문이다(마 참조). 수확량 변화폭을 수치로 추정할 수 없으면 그에 연동된 매출·소득의 최적화도 성립하지 않는다.'),
  B('현재 소득·비용은 ERP에서 실측 기반으로 집계·진단(손익구조·BEP·ROI)되며 이는 "현황 분석"으로서 유효하다. 다만 "무엇을 바꾸면 소득이 는다"는 처방형 최적화와는 구분된다.'),
  B('비용 측면에서 경영비 이상치 탐지와 CAPEX/OPEX 구분 다년 ROI는 아직 미착수·미반영 상태로, 잔여기간 과제로 이월한다(제6장).'),
  B('소득·비용 최적화의 성립은 과업②(다) 현장실증(동일 농가에서 환경 조건을 의도적으로 달리하는 설계)으로 환경→수확량 인과가 확보된 이후 가능하다.'),
  ...
  B('배포 검증은 컨테이너 실체·서버 로컬(localhost)·공개 도메인의 3경로를 대조하여, 사용자에게 실제 도달하는 산출물이 배포분과 일치함을 확인하는 방식으로 수행한다. 세 경로의 응답이 갈리면 연결 경로 이상으로 판정한다 — 도메인 응답만으로는 조용한 불일치를 놓칠 수 있기 때문이다.'),
  B('문의 접수 채널(iiam@iiam.co.kr)을 서비스 진입 화면 하단에 상시 노출하여 현장 피드백 수집 경로를 확보하였다.'),
...
const ch6 = [
  ...
  H('제2절  환경제어 최적화 — 현장실증 설계 (최우선)', HeadingLevel.HEADING_2),
  P('제3장 제2절에서 확인한 바와 같이, 온도·습도·CO₂·일사를 월별로 집계해 수확량과 대조한 범위에서 ∂수확량/∂환경 은 추정되지 않는다(교차검증 R² −0.4~−1.0). 원인은 분석 기법이 아니라 데이터 구조 — 동일 농가가 환경을 크게 변화시킨 이력이 없어 효과를 식별할 변동이 존재하지 않는다. 따라서 다음 설계가 선행되어야 한다.', { after: 110 }),
  B('실증 설계 : 동일 농가·동일 작기 내에서 환경 조건(주야 온도차·CO₂ 시비 수준 등)을 구간별로 달리 적용하고 수확량을 대조한다. 농가 간 비교가 아니라 농가 내 비교여야 시설·품종·재배자 역량의 교란이 제거된다.'),
  B('실증 요인·수준 : 제품이 실제로 제어하는 변수를 처치로 둔다 — 주야 온도차(DIF) 2~3수준, CO₂ 시비 수준 2수준(예: 대기 수준 vs 목표 시비), 필요 시 VPD 밴드 2수준. 나머지 재배 조건(작기·품종·정식일·관수 전략)은 최대한 정렬하여 교란을 줄인다.'),
  B('배치·반복 : 처치는 농가 내 구획(스팬·베드) 단위로 배정하고 작기를 반복한다. 농가 간 비교가 아니라 농가 내·구획 간 비교여야 시설·품종·재배자 역량의 교란이 상쇄된다. "20개소"는 농가 수일 뿐이며, 통계적 검정력은 농가×구획×작기의 반복 관측 수가 결정한다.'),
  B('표본 산정(방법) : 검정력 분석으로 확정한다. 현 관측의 환경–수확량 부분상관은 0.12~0.19(작은 효과, f²≈0.02~0.04)이므로, 작은~중간 효과(f²≈0.06)를 유의수준 .05·검정력 .8로 검출하려면 처치·반복 조합 기준 총 150~250 관측이 필요하다(예시 산정). 확정 표본은 예비조사로 효과 크기·분산을 확인한 뒤 재계산하며, 이를 주관기관과 협의한다.'),
  B('데이터 : 1분 단위 환경 원천은 이미 수집되고 있으며(2021년 딸기 213,188건), 생리학적 지표 산출 파이프라인을 구축해 두었다. 실증 데이터가 확보되면 즉시 재판정 가능하다.'),
  B('협의 : 실증 설계는 주관기관(농촌진흥청) 및 참여 농가와의 협의가 필요하다 — 제3절.'),
  B('그 전까지 환경 처방은 확립된 재배 기준(전략표) 대비 편차 방식을 유지한다. 검증되지 않은 관계를 최적화 근거로 제시하지 않는다.'),

  H('제3절  모델 고도화 계획', HeadingLevel.HEADING_2),
  B('폴백 작목(방울토마토·참외)은 CV R²가 음수로, 현 변수 구성으로는 평균 예측을 넘지 못한다. 표본이 근본 제약이다 — 학습 파이프라인은 농가–연도 표본이 150 미만이면 과적합 방지를 위해 강력한 예측 변수(직전 수확량 계열)를 비활성화하므로, 참외(84)·방울(103)은 성능 상한이 낮다. 과업① 20개소를 3~4개 작기 반복 관측하여 농가–연도 표본을 이 문턱 이상으로 끌어올린 뒤 변수 재설계를 병행한다.'),
  ...
  P('가. 고도화의 선행조건 — 알고리즘이 아니라 입력 정보', { bold: true, after: 80 }),
  P('당해연도 점검 결과, 하이퍼파라미터·모델선택 등 코드 측면의 조정으로 얻을 수 있는 개선은 유의미하지 않았다(파프리카·완숙토마토를 대상으로 시도, 순이득 0.03 미만). 또한 동일한 환경 센서 자료를 더 축적하는 방향도 한계가 있다 — 관측 자료에서 환경–수확량의 부분상관이 0.12~0.19로 작고 농가 역량과 교란되어 있기 때문이다. 따라서 고도화의 실질 선행조건은 분석 기법이 아니라 현재 수집하지 않는 종류의 정보를 확보하는 데 있다.', { after: 110 }),
  TBL([700, 2500, 3600, 2838], [
    ['우선', '확보할 외부 정보', '근거 (측정된 병목)', '기대 효과'],
    ['1', '품질·등급 (당도·특품율·크기)', '매출 오차의 주된 원인은 단가이며 단가는 등급이 결정한다. 현 모델은 수량(kg/m²)만 예측한다', '수량 예측을 소득 예측으로 확장'],
    ['2', '농가별 실경영비 13항목', '비용이 계수표 기반이어서 소득(매출−비용)의 검증 자체가 불가하다', '소득 모델 검증 성립'],
    ['3', '가격 형성 선행지표 (도매 반입량·수입량·산지 작황)', '시세 자료는 결과만 제공하며 예측에 필요한 선행 정보가 없다', '출하 시점 최적화 정밀화'],
    ['4', '근권·양액 실측 (급배액 EC·함수율·배액률)', '관수 기준(P1~P6)이 규칙으로만 존재하고 수확량 변수에 반영되지 않는다', '생리 기반 변수 추가'],
  ...
  B('위 6개 항목 중 1·2·3·5·6은 과업①(2차년도 소득자료 조사 20개소)의 조사 항목과 직접 연결된다 — 조사 설계 단계에서 이들 항목이 확보되도록 반영하면 모델 고도화가 함께 진행된다.'),
  B('환경제어 최적화는 예외다 — 외부 정보를 추가로 확보해도 관측 자료인 한 효과가 식별되지 않으므로, 제2절의 처치 배정 실증이 유일한 경로다.'),

  H('제4절  주관기관 협의 필요 사항', HeadingLevel.HEADING_2),
  B('1년차 조사 유의사항의 플랫폼 반영 범위 확정(과업③).'),
  B('조사 항목–경영성과 매칭율·민감도의 정량적 목표 설정 방식(델파이·부분상관) 협의.'),
  B('★ 환경제어 최적화 실증 설계 협의 — 동일 농가 내 환경 처치 구간 배정. 현 관측 데이터로는 환경→수확량 효과가 식별되지 않아 실증이 유일한 경로다(제2절).'),
  B('과제협의회 기능별 분과(조사설계·데이터수집·정제·AI분석) 운영 일정 협의.'),
  B('통계조사 확대 방안 및 경영의사결정지원 기술의 현장활용 확대 방안 보완.'),

  H('제5절  활용·확산 및 사업화 방안', HeadingLevel.HEADING_2),
