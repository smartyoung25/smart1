 ...
 *            RFID 미착수. CAPEX/OPEX 다년 ROI 부분 이행.
 *            주간/일일 작업추천은 2026-07 구현(G7) — 일일은 이행, 주간은 작목별
 *            농작업 캘린더 미연동으로 부분.)
...
const summary = [
  ...
  P(`당해연도 주요 실적은 다음과 같다. 첫째, 경영성과 분석 플랫폼(farmingsight.org)을 ${S.screens}개 화면·${S.routers}개 API 라우터·${S.services}개 도메인 서비스 규모로 구축하여 상시 운영 중이다. 둘째, 데이터 축적 단계에 따라 정확도와 신뢰성을 양립시키는 3계층(농진청 표준→광역 학습→농장 맞춤) 예측 체계를 구현하고, 주요 시설작목의 수확량 예측모델을 검증 산출물(stage2_meta) 기준으로 평가하였다. 검증 게이트를 단일 정책 모듈로 통합하고 교차검증 설명력(CV R²)을 판정 기준으로 확정한 결과, 딸기·오이는 서비스 적용, 파프리카·완숙토마토는 조건부, 방울토마토·참외는 상위 계층 폴백으로 판정하였다. 이 과정에서 검증 지표 산출의 결함(학습 오차를 검증값으로 사용)을 발견하여 정정하였다(제3장 제2절).`, { after: 170 }),
  ...
  P('다섯째, 계획서 과업④의 주간/일일 작업추천 시스템을 규칙 기반으로 구현하였다. 관수 P1~P6 Period와 환경관리 전략표 목표 대비 실측 편차를 근거로 오늘 해야 할 작업을 제시하며, 각 작업에 판단 근거를 함께 표시한다. 외부 LLM 에 의존하지 않아 비용·장애와 무관하게 상시 동작한다.', { after: 170 }),
  P('한편 계획 대비 미이행 항목이 존재한다. 경영비 이상치 탐지·알림 서비스와 RFID 활용방안은 미착수이며, CAPEX/OPEX 구분에 따른 다년 ROI 추정과 작업추천의 주간 기능(작목별 농작업 캘린더)은 부분 이행 상태이다. 또한 보안조치 이행계획 중 개인정보 비식별화·암호화는 미적용 상태로, 과업①의 소득자료 조사로 농가 개인정보가 수집되기 전까지 완료가 필요하다. 해당 항목은 제6장 잔여기간 추진계획에 반영하였다.', { after: 170 }),
...
const m2rows = F.m2.map((r) => [
  r.crop, r.cv_r2 == null ? '—' : r.cv_r2.toFixed(3),
  r.cv_mape == null ? '—' : `${r.cv_mape.toFixed(1)}%`,
...
const ch3 = [
  ...
  TBL([2600, 1700, 5338], [
    ['판정', '조건', '조치'],
    ['검증 적용(serve)', `CV R² ≥ ${G.cv_r2_min}`, '2계층(농장 맞춤) 모델을 서비스에 적용'],
    ['조건부(conditional)', `${G.cv_r2_fallback} < CV R² < ${G.cv_r2_min}`, '서비스하되 데이터 확충·재학습 보완 대상'],
    ['폴백(fallback)', `CV R² ≤ ${G.cv_r2_fallback} 또는 검증 지표 없음`, '평균 예측보다 나쁨 → 상위 계층(농진청 표준/광역)으로 자동 폴백'],
  ]),
  T('M2 수확량 모델 검증 게이트 판정 기준 (단일 출처: pipeline/gates.py)'),
  B(`판정은 교차검증(CV) 지표로만 한다. 학습(in-sample) 오차는 판정에 쓰지 않는다 — 학습 데이터로 그대로 예측한 값이라 낙관적이기 때문이다.`),
  B(`과적합 가드 : |train R² − CV R²| > ${G.overfit_gap} 이면 조건부로 강등하여 과적합 모델의 서비스 적용을 차단한다.`),
  B(`소표본 경고 : n_train < ${G.n_min} 인 작목은 경고 플래그를 부여하고 농장단위 LOGO(Leave-One-Group-Out) 교차검증 재판정 대상에 포함한다.`),
  P('※ MAPE를 판정 기준에서 제외한 이유', { bold: true, after: 80 }),
  B(`MAPE는 월별(monthly) 모드에서 구조적으로 과대 산출된다 — 수확 초·말기 수확량이 0에 가까워 작은 값으로 나누기 때문이다. 딸기의 CV MAPE ${(F.m2.find(x=>x.crop==='딸기')||{}).cv_mape}%는 모델 부실이 아니라 지표 특성이며, 같은 모델의 CV R²는 ${(F.m2.find(x=>x.crop==='딸기')||{}).cv_r2}로 유효한 설명력을 보인다.`),
  B('따라서 월별 모드(딸기·파프리카)와 연간 모드(그 외)를 MAPE로 비교하는 것은 성립하지 않는다. R²는 두 모드에서 해석이 안정적이므로 주 지표로 채택하였다.'),
  B(`CV MAPE는 참고·경보 지표로 유지한다(${G.mape_advisory}% 초과 시 경보 플래그). 0 근처 표본에 견디는 지표(WAPE 등) 도입 시 재검토한다.`),
  ...
  TBL([1400, 1500, 1600, 1200, 1900, 2038], [
    ['작목', 'CV R² (판정)', 'CV MAPE (참고)', '표본수(n)', '모델', '판정'],
    ...m2rows,
  ...
  T('작목별 수확량(M2) 모델 검증 결과 (models/artifacts/*/stage2_meta.json 권위값·교차검증 기준)'),
  ...(() => {
    const g = (c) => F.m2.find((x) => x.crop === c) || {};
    const [sb, cu, pa, to, ch, me] = ['딸기', '오이', '파프리카', '완숙토마토', '방울토마토', '참외'].map(g);
    return [
      B(`딸기(CV R² ${sb.cv_r2})·오이(${cu.cv_r2})는 기준(${G.cv_r2_min})을 충족하여 서비스 적용 — 현장 수확량 예측·의사결정 활용 가능.`),
      B(`파프리카(${pa.cv_r2})·완숙토마토(${to.cv_r2})는 설명력이 기준에 미달하여 조건부 — 서비스하되 변수 재설계·데이터 확충 대상이다.`),
      B(`방울토마토(${ch.cv_r2})·참외(${me.cv_r2})는 CV R²가 음수로, 평균값 예측보다 나쁘다 → 상위 계층(농진청 표준/광역)으로 폴백하여 예측 안정성을 유지한다.`),
      B(`표본수가 경고 임계(n<${G.n_min}) 미만인 작목(오이 n=${cu.n_train})은 농장단위 LOGO 교차검증 재판정 대상이다(제6장).`),
    ];
  })(),
  P('※ 본 표의 지표는 교차검증(CV) 값이다', { bold: true, after: 80 }),
  B('당해연도 점검에서 검증 지표 산출의 결함 두 건을 발견하여 정정하였다. ① 게이트가 학습(in-sample) MAPE로 판정하고 있었다(학습 데이터로 그대로 예측한 값). ② 소표본 작목은 교차검증 없이 학습 R²를 검증값으로 기록하고 있었다.'),
  B('정정 결과 검증 적용·조건부·폴백의 작목 구성은 종전과 동일하였으나, 판정 근거가 교차검증 실측으로 대체되었다. 정정 과정에서 모델 선택도 개선되어 오이는 표본 61·CV R² 0.386, 파프리카는 월별 모드 858표본으로 재학습되었다.'),
  B('참고로 학습 MAPE와 CV MAPE의 괴리는 컸다(딸기 17.8% → 107.3%). 학습 오차를 검증값으로 보고하지 않도록 게이트·보고 경로를 단일화하였다.'),
  ...
  TBL([1200, 2600, 1300, 4538], [
    ...
    ['5단계', '지속운영 및 리포트 제공', '이행', '컨설팅 리포트 화면 + 일일·주간 작업추천(G7) 구현 — 매일 해야 할 작업을 근거와 함께 제시'],
  ...
  TBL([2800, 1300, 5538], [
    ...
    ['주간/일일 작업추천 시스템', '부분', '일일 구현(G7) — 관수 P1~P6 Period·환경 전략표 편차·기록 점검을 근거와 함께 제시.\n주간은 요일별 점검 루틴만 — 작목별 농작업 캘린더(적엽·유인 등) 미연동'],
  ...
  H('4. 주간·일일 작업추천 (G7)', HeadingLevel.HEADING_3),
  P('계획서가 제시한 "주간/일일작업추천시스템"을 규칙 기반으로 구현하였다. 농가가 오늘 무엇을 해야 하는지를 판단 근거와 함께 제시한다.', { after: 130 }),
  TBL([2000, 7638], [
    ['구분', '내용'],
    ['일일 추천', '① 관수 — 현재 P1~P6 Period의 목표(배액률·EC·dry-back) 대비 실측 편차\n② 환경 — 전략표의 지금 생육시기·구간 목표 대비 편차 처방(난방·환기·CO₂·VPD)\n③ 기록 — 비어 있는 실측 항목 보완 유도'],
    ['주간 추천', '요일별 점검 루틴(주간 목표 리뷰·급액 EC/pH 점검·기록 완결성·차주 관수 전략)'],
    ['근거 제시', '각 작업에 판단 근거를 함께 표시 — 어느 Period·어느 목표 대비 몇 만큼 벗어났는지'],
    ['동작 조건', '외부 LLM 불필요 — 비용·장애와 무관하게 상시 동작'],
  ]),
  T('일일·주간 작업추천 구성'),
  B('근거 제시 : "왜 지금 이 작업인가"를 함께 보여준다. 근거 없는 지시는 농가가 신뢰하지 않고, 잘못된 조치로 이어질 수 있다.'),
  B('실측이 없으면 추천을 만들지 않는다 — 대신 ‘기록하기’를 제시하고, 추천이 적은 이유가 ‘문제 없음’이 아니라 ‘근거 없음’임을 화면에 밝힌다.'),
  B('단일 출처 : P1~P6 Period 목표값은 제품 코드에서 자동 추출한 구조체를 읽는다. 수기 사본을 두지 않아 규칙과 구현이 어긋나지 않는다.'),
  B('한계(명시) : Period 판정은 현재 시각 기준이다. 계획서의 트리거 원칙(일사 적산 우선)에 따라 실시간 일사 적산 연동이 필요하며, 이 사실을 화면에 표기한다.'),
  B('한계(명시) : 주간 추천은 관수·환경·기록 항목만 제공한다. 작목별 농작업 캘린더(적엽·유인·수분 등)는 보유 지식이 아니어서 지어내지 않았다 — 교재 지식베이스 연계로 확장 예정(제6장).'),
  ...
  H('6. 실증 서비스 운영', HeadingLevel.HEADING_3),
  ...
  H('7. 데이터 보안 조치 이행', HeadingLevel.HEADING_3),
  P('계획서 보안조치 이행계획에 따른 현황이다.', { after: 130 }),
  TBL([2600, 1300, 5738], [
    ['보안 조치 계획', '이행', '현황'],
    ['데이터 접근 통제', '이행', '사용자 권한(역할) 기반 접근 제한. 공개 데모 환경은 관리자 쓰기 전면 차단'],
    ['개인정보 비식별화·암호화', '조치 중',
     '무인증 공개 응답의 농장 식별자는 솔트 해시로 익명화 적용.\n'
     + '가입자 연락처(전화·이메일)는 현재 평문 저장 상태로, 비식별화·암호화 미적용 — 제6장 제1절 조치계획 반영'],
...
const ch5 = [
  ...
  TBL([2600, 1400, 3800, 1838], [
    ...
    ['④-2 주간/일일 작업추천', '「기재」', '일일 구현(관수 Period·환경 편차·기록, 근거 제시). 주간은 점검 루틴만 — 작목별 농작업 미연동', '부분 달성'],
...
const ch6 = [
  ...
  TBL([2800, 4900, 1938], [
    ['미이행 항목', '조치계획', '시기'],
    ['개인정보 비식별화·암호화', '가입자 연락처(전화·이메일)의 비식별화(해시·마스킹) 및 저장 시 암호화 적용. 계획서 보안조치 이행계획 사항으로, 2차년도 소득자료 조사(과업①)로 농가 개인정보가 수집되기 전까지 완료 필요', '「기재」'],
    ...
    ['작업추천 고도화', '① 일사 적산 실시간 연동으로 Period 판정 정밀화(현재 시각 기준) ② 작목별 농작업 캘린더를 교재 지식베이스와 연계해 주간 추천 확장', '「기재」'],
  ...
  B('폴백 작목(방울토마토·참외)은 CV R²가 음수로, 현 변수 구성으로는 평균 예측을 넘지 못한다. 실측 데이터 확충과 변수 재설계를 병행한다.'),
  B(`조건부 작목(파프리카·완숙토마토)의 설명력을 기준(${G.cv_r2_min}) 이상으로 끌어올리기 위한 변수 재설계 — 특히 완숙토마토는 표본(n=177)이 충분하므로 변수 문제로 판단된다.`),
  B(`소표본 작목(n<${G.n_min})의 농장단위 LOGO 교차검증 재판정.`),
  B('평가 지표 보완 : 0 근처 표본에 견디는 오차 지표(WAPE 등) 도입 — 월별·연간 모드를 동일 기준으로 비교하고, 서빙 블렌딩 임계도 함께 정합화한다.'),
