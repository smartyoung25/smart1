...
const summary = [
  ...
  P('중요한 확인 사항으로, 환경제어 최적화의 전제인 환경→수확량 관계는 현재 데이터로 추정되지 않는다(교차검증 R² −0.4~−1.0). 원인은 분석 기법이 아니라 데이터 구조로, 동일 농가가 환경 설정을 크게 변화시킨 이력이 없어 효과를 식별할 변동이 존재하지 않는다. 따라서 계획서 과업②(다)의 현장실증 설계가 선행되어야 하며, 그 전까지 환경 처방은 확립된 재배 기준(전략표) 대비 편차 방식을 유지한다. 검증되지 않은 관계를 최적화 근거로 제시하지 않는다(제3장 제2절·제6장 제2절).', { after: 170 }),
...
const ch3 = [
  ...
  TBL([3400, 1500, 4738], [
    ...
    ['환경제어 최적화', '부분', '전략표 목표 대비 편차 처방·관수 P1~P6·에너지 절감은 구현.\n데이터 기반 환경→수확량 최적화는 성립하지 않음(하단 검증 참조)'],
  ], [AlignmentType.LEFT, AlignmentType.CENTER, AlignmentType.LEFT]),
  T('과업②(다) 현장실증·최적화 이행 현황'),

  P('마. 환경→수확량 최적화 검증 — 현 데이터로는 성립하지 않음', { bold: true, after: 80 }),
  P('환경제어 최적화는 "환경 설정을 바꾸면 수확량이 얼마나 변하는가(∂수확량/∂환경)"를 알아야 성립한다. 이를 당해연도에 검증한 결과, 현재 확보된 데이터로는 이 관계를 추정할 수 없음을 확인하였다.', { after: 110 }),
  ...
  B('따라서 환경→수확량 최적화는 계획서 과업②(다)의 현장실증을 통해서만 가능하다 — 동일 농가에서 환경 조건을 의도적으로 달리하는 설계가 필요하다. 검증되지 않은 관계를 근거로 환경 처방을 제시하면 농가를 잘못된 조치로 이끌 수 있으므로, 해당 모델은 서비스에 반영하지 않았다.'),
...
const ch6 = [
  ...
  P('제3장 제2절에서 확인한 바와 같이, 환경→수확량 관계는 현재 데이터로 추정할 수 없다(교차검증 R² −0.4~−1.0). 원인은 분석 기법이 아니라 데이터 구조 — 동일 농가가 환경을 크게 변화시킨 이력이 없어 효과를 식별할 변동이 존재하지 않는다. 따라서 다음 설계가 선행되어야 한다.', { after: 110 }),
