...
const ch3 = [
  ...
  TBL([2600, 3400, 3638], [
    ...
    [{ t: '작목 차이', bold: true }, { t: '딸기=방울=완숙=참외 동일', fill: OLD_FILL }, { t: '참외 94.5 vs 딸기 135 원/㎡·월 (시설완비도 차등)', fill: NEW_FILL }],
...
const ch4 = [
  ...
  B('이후 : 측면 보온스크린만 ✕ → 계수 0.92, 감가 1,341,360. 보온 투자 여지가 공백으로 노출.'),
  ...
  B('이후 : 측창·관수관비 ✕ → 계수 0.82, 감가 1,195,560. 자동화 공백 최대 → 노무 의존 구조 노출.'),
  B('활용 : 환기·관수 자동화 투자 여지 최대. 규모 효과 큼 → 자동화 CAPEX(감가+전기) vs 노무·수율 ROI가 핵심.'),

  H('제4절  참외 (강석구) — 반촉성·저투자·관수 고장', HeadingLevel.HEADING_2),
  B('이전 : 동일 템플릿(1,458,000) → 반촉성·최소 시설인 참외가 딸기와 같은 투자비로 잡혀 소득 왜곡.'),
  B('이후 : 보온·차광 없음, 측창만 ○, 관수관비 고장 → 계수 0.70, 감가 1,020,600(최소). 저CAPEX 특성 반영.'),
...
const ch5 = [
  ...
  TBL([2400, 3400, 3838], [
    ['관점', '이전', '이후'],
    ['품목 구분', { t: '4작목 CAPEX 동일(템플릿)', fill: OLD_FILL }, { t: '시설 실구성으로 차등(참외 −30% vs 딸기)', fill: NEW_FILL }],
  ...
  B('투자 의사결정 지원 — 신규 CAPEX의 (연 감가상각 + 유발 OPEX 증분) vs (노무 절감·수율·품질 편익) ROI 시뮬레이션. 품목별 최우선 투자를 자동 제안(완숙 환기·관수, 방울 측면보온, 참외 관수 교체).'),
