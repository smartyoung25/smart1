...
const _domAcq = DOMAINS.map(() => 0); let _domTot = 0;
Object.values(REG.crops || {}).forEach(d => (d.farms || []).forEach(fm => (fm.assets || []).forEach(a => { const v = a.acq_krw || 0; _domAcq[_domIdx(a.asset)] += v; _domTot += v; })));
const _domPct = i => _domTot ? (_domAcq[i] / _domTot * 100).toFixed(1) + '%' : '—';
...
const ch3 = [
  ...
  H('제4절  기능별 운영관점', HeadingLevel.HEADING_2),
  P('개선 구조는 자산 분류(무엇을)·비용 종류(얼마)에 더해, 플랫폼 최적화 도메인(환경제어·관수·재배·병해·수확·운영)의 관점으로도 조망할 수 있다. 각 도메인이 어떤 CAPEX를 두고, 어떤 OPEX를 유발하며, 어떤 지표로 관리되고, 어느 화면과 연결되는지를 정리한다.', { after: 100 }),
  TBL([1500, 2500, 1900, 2200, 1538], [
    ['운영 도메인', 'CAPEX 자산군', '연동 OPEX', '핵심 지표(KPI)', '플랫폼 연계'],
    ...DOMAINS.map((d) => [{ t: d[0], bold: true }, d[1], d[2], d[3], { t: d[4], color: '1F7A4D', bold: true }]),
  ], [AlignmentType.LEFT, AlignmentType.LEFT, AlignmentType.LEFT, AlignmentType.LEFT, AlignmentType.LEFT]),
  T('기능별 운영 도메인 × CAPEX/OPEX/KPI/플랫폼 연계'),
  BOX(`20농가 실 자산등록부(251자산)를 도메인으로 배분한 취득가 점유율 — 재배·생육 ${_domPct(2)}(온실 구조체 중심) · 운영 ${_domPct(5)} · 관수·양액 ${_domPct(1)} · 환경제어 ${_domPct(0)} · 수확·출하 ${_domPct(4)} · 병해·방제 ${_domPct(3)}. 구조체(하우스)가 최대 CAPEX이므로 재배 비중이 크며, OPEX는 3대분류(재료비·경비·노무비), KPI는 제3절 산식·G3 관수 P1~P6과 연결된다.`, { label: '실측 배분' }),
];
