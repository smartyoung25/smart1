...
def load_production_with_revenue(crop_ko: str) -> pd.DataFrame:
    ...
    logger.info("  수익 데이터: %d행 (yield 중앙=%.3f, price 중앙=%.0f)",
                len(prod), prod["yield_per_m2"].median(), prod["price_actual"].median())
    return prod
...
def train_stage3(crop_ko: str) -> dict | None:
    ...
    logger.info("  가격 통계: 중앙=%.0f p25=%.0f p75=%.0f 원/kg",
                price_med, price_p25, price_p75)

    # 피처: log(yield), log(price_stats_median), month
    X = np.column_stack([
        np.log1p(prod["yield_per_m2"].values),
        np.full(len(prod), np.log1p(price_med)),
        prod["month"].astype(float).values,
    ])
    y_log = np.log1p(prod["revenue_per_m2"].values)
    ...
    if n < 10:
        ...
        mape_pure = float(mean_absolute_percentage_error(
            ...
            prod["yield_per_m2"].values * price_med
        ) * 100)
    ...
    X_s = np.column_stack([
        ...
        np.full(len(prod_sorted), np.log1p(price_med)),
    ...
    return {
        "crop_ko": crop_ko, "crop_en": config.crop_en,
        "ridge": ridge,
        "feature_names": ["log_yield", "log_price_median", "month"],
        "price_median": price_med, "price_p25": price_p25, "price_p75": price_p75,
