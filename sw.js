 ...
 */
const CACHE = 'kaasa-smartos-v49';  // ★ v49: C13 AI비서 입력 이상치 선제 알림';  // ★ v48: 입력 이상치 자동 플래그(data_anomaly)→C17 데이터 품질 점검';  // ★ v47: F4 노지 관개량 이상치 경고';  // ★ v46: G3 관수 입력 이상치 경고(EC·pH 범위 가드)';  // ★ v45: G2 환경 입력 이상치 경고(허용범위 차단·정상범위 확인)';  // ★ v44: G2 환경 입력 이력(수정 추적) 기록·표시';  // ★ v43: G2 환경 실측값 입력·수정(수동 입력→화면 반영)';  // ★ v42: G2 환경 출처배지 정직화(등록 센서 연동 반영)·중복배지 정리';  // ★ 버전 변경 시 구 캐시 자동 삭제 (v41: C16 표준변수 한글 라벨)
const CORE = [
  '/intro', '/index.html',
