# NEXT — 다음 세션 시작점
> 생성: 2026-06-17 / 갱신: 2026-06-18 / 코드 마지막 커밋: `472a8f2`

## 현재 상태 (1줄)
회원가입 흐름 단락 해소(register farm_id 발급→C1 동일 farm_id·온보딩 JWT 권위·로그인 온보딩 라우팅). 기자재(C16) 전 과정 완성. C24 시공업체 찾기. 터널 자동복구 정상. SW v58.

## 이번 세션 목표 (1개만)
[ ] 미정 — 사용자 지시 대기 (기자재 흐름 완료, 다음 영역 선택 필요)

## 다음 3개 작업 (후보)
1. 카탈로그 공종 매핑 보강: 2964/3071 "기타" → 기종명 기반 공종 재매핑(원본 DB 한계 보완)
2. C17 종합진단에 보유장비 평균등급 카드 정식 연계(현재 C16에서 버튼 링크만)
3. 토마토 M1: ERA5 실측 CSV 확보 시 재학습(보류 중)
