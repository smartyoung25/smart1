...
★ 주의(2026-07): M2(수확량)의 **런타임 게이트는 pipeline/gates.py 가 단일 출처**다.
  이 모듈은 M1·M3·M4·M5 를 포함한 선언적 스펙으로, 현재 서빙 경로(api/·pipeline/)에서는
  사용되지 않고 테스트에서만 참조된다. 아래 M2/STAGE2_* 임계값은 구 기준(35%)이며
  런타임 판정과 다르다. 새 게이트 로직을 여기에 추가하지 말 것 — pipeline/gates.py 를 사용.
...
GATES: dict[str, dict] = {
    ...
    # 4-Stage 파이프라인 게이트
    # Rule-A 완화 기준 (2026-05-22):
    # mean ≥ -0.25 / min ≥ -0.30 OR Trimmed Mean PASS
    # 구 기준(mean≥-0.20 / min≥-0.25) 에서 완화 — 농진청 소규모 데이터 N<200 작목 허용
    "STAGE1_CV_R2":      {"metric": "cv_r2_mean",  "threshold": -0.25, "op": "gte",
