...
class TestWorldBankPriceAdapter:
    ...
    def test_import_ok(self):
        ...
        assert "딸기" in _CROP_TO_SERIES
        assert "오이" in _CROP_TO_SERIES
    ...
    def test_get_m3_correction_factor_unknown_crop(self):
        """미등록 작목 → 기본 세계가격(1.0 USD/kg) 대비 보정계수 반환."""
        from adapters.worldbank_price_adapter import get_m3_correction_factor
        cf = get_m3_correction_factor("미등록작목xyz", domestic_price_krw_kg=3000.0)
        # 미등록 작목: world_usd=1.0 → world_krw≈1320 → ratio=3000/1320≈2.27 → clip→2.0
        assert 0.5 <= cf <= 2.0  # 0.5~2.0 범위 내이면 OK
...
class TestOptimizeBayesian:
    ...
    def test_bayesian_fallback_without_yield_fn(self):
        ...
        from unittest.mock import patch
        with patch("engine.profit_optimizer.optimize", return_value=[]):
            env = EnvState(farm_id="farm_001",
                           values={"temp_internal": 22.0, "humidity_int": 70.0,
    ...
    def test_bayesian_with_mock_yield_fn(self):
        ...
        env = EnvState(farm_id="farm_001",
                       values={"temp_internal": 18.0, "humidity_int": 70.0,
        ...
        assert isinstance(recs, list)
        for r in recs:
            # Recommendation은 action_ko, profit_delta 등의 필드를 가짐
            assert hasattr(r, "action_ko") or hasattr(r, "profit_delta")
    ...
    def test_cucumber_mapping_in_optimize(self):
        ...
        env = EnvState(farm_id="farm_001",
                       values={"temp_internal": 25.0, "humidity_int": 75.0,
                               "co2_ppm": 800.0, "solar_rad": 100.0, "soil_temp": 20.0})
        try:
            recs = optimize(
                farm_id="farm_001", tier=FarmTier.MANUAL,
                current_env=env, crop_ko="오이",
                area_m2=1000.0, horizon_days=30,
            )
            assert isinstance(recs, list)
        except KeyError as e:
            pytest.fail(f"오이 매핑 누락 KeyError: {e}")
        except Exception:
            pass  # M2 모델 없는 등 다른 오류는 허용
