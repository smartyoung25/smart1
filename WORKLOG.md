...
## 2026-06-09 (PM) — 진단·역량·클러스터 + 실데이터 + 시각화/UX 강화

### FEAT (신규 기능)
| 항목 | 결과 |
|------|------|
| 16일 장기예보(Open-Meteo) 실연동 + F3 스트립 | ✅ ET₀·일사·강수확률·재해경보 |
| 16일 재해→F1 노지 선행 의사결정 카드 | ✅ D-day·처방·원탭기록 |
| 현장컨설팅 6대 영역 진단(C17) + 현장문진(C18) | ✅ 센서·구동기·재배·경영·유통·데이터 |
| 진단 결과 → C14 리포트·C4 전문가 연결 + 회차 추세 | ✅ |
| 역량 단계별 핵심 서비스 큐레이션(C19, 4단계) | ✅ 시설/노지 분기 |
| 노지 클러스터 작황 모니터링(F8) 무센서 위성·기상 | ✅ 위치특정 이상알림 |
| 위성 NDVI 프록시→sentinel-2 자동전환 | ✅ SATELLITE_NDVI_URL |
| 다중농가 클러스터 관제(C20) 723농가 + 보고서PDF + 일괄알림 | ✅ |
| 진단·월간 결과보고서 PDF(c17_report·c14_report_pdf) | ✅ |
| C13 챗봇 고도화(진단·역량·작황·경영전략 컨텍스트) | ✅ LLM키 없이 |

### REALDATA (제주 실데이터 주입, PII 제외 집계·지역매칭 자동전환)
| 연동 | 원천 | 결과 |
|------|------|------|
| 흙토람 토양 | 토양검정 13,842필지 | ✅ /field/soil naas_soil_real |
| 팜맵 필지 | 농경지지도 276,491필지 | ✅ /field/parcels·F8 farmmap_real |
| 소득조사 벤치마크 | 102농가 | ✅ /benchmark 실비교군(소득률 상위25% 66.2%) |
| 병해충 예찰→G5/F6 | 감귤 예찰 | ⏳ 파이프라인 완성(소형 parquet 인라인반환→적재대기) |
| M2 재학습 | 생육조사 | ⛔ 수확량 타깃·연결키 부재로 불가(사유 문서화) |

### 시각화 10건 (의존성0 SVG/CSS)
C3 우수농가바·실데이터배지 / C17 6대영역 레이더 / F8 NDVI히트맵 / G3 일일WC곡선(P1~P6) / C14 전월대비 다이버징 / G2 온습도·VPD추이 / G6 신뢰구간밴드 / F3 16일 기온·강수 / G4 초장추세
...
- 효과성·효율성: 촉각 피드백 scale(.975)·헤더버튼 40×40·앵커 scroll-margin·hover 어포던스
- 전역 오프라인/연결 감지 배너(현장 통신 불안정 대응)

### 여정 무단절 정합 (단절 6건 해소)
가입→세팅→(시설:기자재→진단 / 노지:클러스터작황)→역량→운영→경영전략. flow.html 9단계 현행화.
