...
BASE_TEMP = {
    ...
    # 노지
    "콩":10.0, "무":5.0, "배추":5.0, "양배추":5.0, "마늘":3.0, "양파":5.0, "고추":10.0, "가지":10.0,
    # 제주노지
    "감귤":13.0,
}
OPT_TEMP_MAX = {
    ...
    # 노지
    "콩":30.0, "무":20.0, "배추":20.0, "양배추":22.0, "마늘":22.0, "양파":22.0, "고추":30.0, "가지":32.0,
    # 제주노지
    "감귤":25.0,
}
...
if era5 is not None:
    ...
    # 이상기상 지수(solar_anomaly_idx, gdd_anomaly_idx)는 역효과 확인 → 미포함
    # ERA5 절대값 4개(t_ext/solar/rain/gdd)만 유지
    n_era5_matched = df["era5_t_ext"].notna().sum()
    print(f"  ERA5 피처 병합: {n_era5_matched}/{len(df)}행")
else:
...
# co2_x_solar 교호 피처: 역효과 확인(완숙 악화) → 제외
# prev_yield: NaN 45%+ 작목에서 과적합 → 제외
